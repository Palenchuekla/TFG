...
- [code/ML_experiment_example](./code/ML_experiment_example.ipynb) →  ML experiment example. All the ML results of the thesis were generated executing a similar notebook. The example uses an SVM classfier, but just by changing the used model and the hyperparameters, the code is functional for a logistic regressor and a random forest classifier.
